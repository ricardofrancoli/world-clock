...
const getClock = async (timezone) => {
	try {
		...
		timezoneLocal.textContent = `${res.data.timezone} ${getLocationEmoji(
			res.data.timezone
		)}`;
	} catch (err) {
...
const getTimezone = async (lat, lng, population) => {
	try {
		const res = await axios.get(
			'http://secure.geonames.org/timezoneJSON?',
			{
				params: {
					lat: lat,
					lng: lng,
...
const startClock = (unixtime, searchOffset) => {
	...
	const timeCount = () => {
		...
		currentTime = new Date(
			(unixtime + (searchOffset - localOffset)) * 1000
		).toLocaleTimeString();
...
const getLocationEmoji = (timezone) => {
	if (timezone.includes('Canary')) {
		// This is here just cos I'm from the Canary Islands and we're really proud of our land!
		return '🇮🇨';
	} else if (
...
const getFactsList = (timezoneId, sunrise, sunset, lat, lng, population) => {
	...
	searchFactsList.innerHTML = `
        <li>It's in the <b>${timezoneId} ${getLocationEmoji(
		timezoneId
	)}</b> timezone</li>
        <li>Population: <b>${new Intl.NumberFormat().format(
			population
		)}</b></li>
        <li>Sunrise is at <b>${sunrise}h</b></li>
